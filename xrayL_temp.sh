...
# 这行代码已更新，以确保获取所有 IPv4 和 IPv6 地址，并排除回环地址
IP_ADDRESSES=($(ip -4 addr show | awk '/inet / {print $2}' | cut -d/ -f1 | grep -v '127.0.0.1'); $(ip -6 addr show | awk '/inet6 / {print $2}' | cut -d/ -f1 | grep -v '::1'))
